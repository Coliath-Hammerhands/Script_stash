...
def parse_episode(filename):
    ...
    name = filename.replace(ext, "").replace("2016", "").replace("2017", "").strip()
    matches = re.findall(pattern, name)
    cleaned_matches = [item.replace("[", "").replace("]", "").strip() for item in matches if item.strip()]
    # print(cleaned_matches)
    episode = [int(x) for x in name.split(" ") if len(x) == 2 and x.isdigit()][0]
    assert episode <= 25, f"Episode number too high {episode}"
    assert cleaned_matches[0] == "NoobSubs", f"It was {cleaned_matches[0]}"
    return f"Berserk S01E{str(episode).zfill(2)}{ext}"
